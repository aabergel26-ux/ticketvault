...
// If we synced within this window, serve straight from Supabase — no Gmail calls.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Synced recently — serve cached tickets straight from Supabase, no Gmail calls.
  const lastSyncMs = user.last_sync_at ? new Date(user.last_sync_at).getTime() : 0;
  if (Date.now() - lastSyncMs < FRESH_WINDOW_MS) {
    try {
      const cached = await getTicketsByUser(user.id);
      return res.json(cached);
    } catch (err) {
      console.error(err);
      return res.status(500).json({ error: 'Failed to load cached tickets' });
    }
  }
